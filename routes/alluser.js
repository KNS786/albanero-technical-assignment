...
router.get('/alluser',async function(req,res){
    ...
    //if user present true provide all db users
      let username=req.body.username;//current login user
     if(username.length > 0){
     let results=get();
      let dballUser=results.db(DB_NAME);     
      let alluser_results=await dballUser.collection(DB_COLLECTION_USER).find({}).toArray();
